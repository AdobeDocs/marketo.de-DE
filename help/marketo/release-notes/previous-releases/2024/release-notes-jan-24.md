...
* **Aktualisierung der Lead-API synchronisieren**: Die [Lead-API synchronisieren](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Leads/operation/syncLeadUsingPOST){target="_blank"}Das Verhalten von wurde in Bezug auf Aktualisierungen der `unsubscribed` -Feld. Jetzt, wenn Sie `null` als Wert ist, ist er gleich der Übergabe eines Werts von `false`.
