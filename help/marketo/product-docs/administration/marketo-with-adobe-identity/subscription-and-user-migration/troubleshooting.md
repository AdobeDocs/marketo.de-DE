---
description: Handbuch zur Fehlerbehebung bei Fehlern bei der IMS-Benutzermigration, z. B. ungültiges Gmail-Zeichen, nicht im Verzeichnis enthaltene Benutzende, inaktive Benutzende und ungültige Domain, mit Auflösungsschritten.
...
**Grundursache**: Wenn ein Federated Account (T3) für die Organisation konfiguriert ist und ein individuelles Konto (T1) bereits für den Benutzer vorhanden ist, schlägt jeder Versuch, ein Nur-Berechtigungskonto (T2e) zu erstellen (falls zutreffend) und mit dem Federated Account zu verknüpfen, mit dem Fehlercode FAILED_TO_CREATE_TYPE2E_USER fehl.

**Auflösungen**:

1. Entfernen Sie den einzelnen Benutzer aus der Adobe-Organisation. _Hinweis: Der Benutzer verliert den Zugriff auf alle Produkte und muss später erneut berechtigt werden. Wenn dies nachteilig ist, wenden Sie sich an den Adobe-Support._
...
**Ursache**: Das Federated-Konto des Benutzers ist entweder inaktiv/deaktiviert oder fehlt in Active Directory (AD).

**Auflösungen**:

_Wenn der Benutzer migriert werden muss_

1. Ein Systemadministrator muss den Kontostatus und die Berechtigungen des Benutzers wiederherstellen.

1. Für SSO-Organisationen mit AD-Synchronisierung:

   * Erstellen Sie den Benutzer über den Identitätsanbieter (IdP).
   * Benutzerdaten mit AD synchronisieren.

1. Für Organisationen ohne SSO-/AD-Synchronisierung:

   * Erstellen Sie das verwaltete Konto neu, um den Status wiederherzustellen.
   * Der Organisationsadministrator bzw. die Organisationsadministratorin muss Rollen und Berechtigungen neu zuweisen.

1. Marketo Engage Admin führt die Migration in der Migrationskonsole erneut aus.
...
* Wenn der Fehler weiterhin besteht, senden Sie einen Support-Fall mit relevanten Details an den [Marketo-Support](https://nation.marketo.com/t5/support/ct-p/Support).
...
Senden Sie ein Ticket für den technischen Support mit relevanten Details für den [Marketo-Support](https://nation.marketo.com/t5/support/ct-p/Support?profile.language=de){target="_blank"}.
