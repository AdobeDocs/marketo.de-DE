---
unique-page-id: 7512607
description: Laden Sie  [!DNL Google AdWords]  Aktivitätsprotokoll von LaunchPoint herunter, um Integrationsprobleme zu beheben.
title: Herunterladen [!DNL GoogleAdwords Activity Log]
exl-id: db62de71-9e9e-46de-a788-3e9e4e7d09e7
feature: Administration
source-git-commit: e894ece3a643113fd3e1d8df9f8addefea5553f5
workflow-type: tm+mt
source-wordcount: '51'
ht-degree: 0%

---

# [!DNL Google Adwords] Aktivitätsprotokoll herunterladen {#download-googleadwords-activity-log}

Dinge passieren. Manchmal müssen Sie das Aktivitätsprotokoll herunterladen, um Probleme zu beheben.

1. Navigieren Sie zum Bereich **[!UICONTROL Admin]**.

   ![](assets/download-googleadwords-activity-log-1.png)

1. Klicken Sie **[!UICONTROL LaunchPoint]**.

   ![](assets/download-googleadwords-activity-log-2.png)

1. Suchen Sie Ihren [!DNL Google AdWords] und klicken Sie auf **[!UICONTROL Aktivitätsprotokoll herunterladen]**.

   ![](assets/download-googleadwords-activity-log-3.png)

1. Eine ZIP-Datei wird auf Ihren Computer heruntergeladen.
