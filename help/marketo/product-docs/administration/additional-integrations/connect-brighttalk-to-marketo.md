...
description: Verbinden Sie Ihren  [!DNL BrightTALK] -Kanal über LaunchPoint mit Marketo mithilfe von Benutzeranmeldeinformationen, die nur APIs sind.
...
>[!NOTE]
>
>An dieser Stelle müssen Sie ein [!DNL API Only User Role] und ein [!DNL API User] einrichten, um die Berechtigungen einzuschränken, die [!DNL BrightTALK] in Ihrer Marketo-Instanz haben werden. Für diese Schritte sind Artikel verfügbar.
...
1. [Erstellen eines API-](/help/marketo/product-docs/administration/users-and-roles/create-api-only-user.md){target="_blank"} mithilfe der [!DNL BrightTALK] API-Rolle, die Sie in Schritt 4 erstellt haben.
...
Nachdem die Anmeldeinformationen authentifiziert wurden, haben Sie [!DNL BrightTALK] offiziell mit Marketo verbunden. Der nächste Schritt besteht darin, zu bestimmen, welche Datenfelder Sie synchronisieren möchten. Wenn Sie dabei Hilfe benötigen, wenden Sie sich an den [BrightTALK-Support](https://www.brighttalk.com/){target="_blank"}.
